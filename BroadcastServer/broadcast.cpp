...
int broadcast_item_ID(int fd_broadcast, int item_id)
{
	...
	broadcast_request_attach(data_buf, sizeof(data_buf), rsn++, 0, item_id,
			gSetting_system.Broadcast_volume);

	syslog(LOG_DEBUG, "REQ:\t<Play> %d.mp3\n", item_id);
	write(fd_broadcast, data_buf, sizeof(data_buf));

	ret = dev_read_timeout(fd_broadcast, data_buf, sizeof(data_buf), 500);
	if (ret > 0)
	{
		doc = xmlReadMemory(data_buf, ret, NULL, NULL, 1);
		if ( NULL == doc)
		{
		...
		if (xmlStrcmp(root_node->name, BAD_CAST "AUDIO"))
		{
		...
		while ( NULL != cur_node)
		{
			if (!xmlStrcmp(cur_node->name, BAD_CAST "RESULT"))
			{
				led_msg = xmlNodeGetContent(cur_node);
				sprintf(info,"ANSR:\t<Result> %s\n", led_msg);
				log_send(info);
				xmlFree(led_msg);
			}
