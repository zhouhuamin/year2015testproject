	...
	while ( NULL != cur_node)
	{
		...
		else if (!xmlStrcmp(cur_node->name, BAD_CAST "BROADCAST_VOLUME"))
		{
			...
			if (0 == interger_check((char*) str_tmp))
			{
				...
				if(gSetting_system.Broadcast_volume > 120)
				{
					gSetting_system.Broadcast_volume = 120;
				}
			}
			else
			{
				gSetting_system.IC_read_block_NO = 90;
			}
			xmlFree(str_tmp);
		}
...
int save_setting_to_xml(sSystem_setting *p_setting)
{
	...
	sprintf(str_tmp, "%d", gSetting_system.Broadcast_volume);
	xmlNewChild(root_node, NULL, BAD_CAST "BROADCAST_VOLUME",
	BAD_CAST (str_tmp));
