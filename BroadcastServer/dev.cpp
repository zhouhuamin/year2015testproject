...
static void broadcast_msg_queue_init(void);
static int broadcast_msg_send(u8 cmd, u32 param, char *led_msg);
static int msg_recv_broadcast(u32 *p_item_id, char *led_msg);
...
//	doc = xmlReadMemory(p_inter, xml_buf_len, NULL, "GB2312", 1);
...
//			LOG(log_str);
	...
	//broadcast_msg_send(0, led_code, (char*) gb_buf);
...
int broadcast_trans_handle(int fd_dev)
{
	...
	if (ret > 0)
	{
            ...
            boost::timer tt;
//            int nTmpSocket = 0;
//            dev_open_handle(nTmpSocket);
//            //dev_connect_handle(nTmpSocket);
//            broadcast_item_ID(nTmpSocket, item_id);
//            close(nTmpSocket);
            
            	syslog(LOG_DEBUG, "==================================Try Again===============================\n");
                
                int iCount;

                char exeFullPath[256];

                iCount = readlink("/proc/self/exe", exeFullPath, 256);
                ...
                //sprintf(cpPath, "%sBroadcast_test", strFullExeName.c_str());
                sprintf(cpPath, "%sPlayer_test.py", strFullExeName.c_str());
                
		char sh_cmd[512] = {0};
		sprintf(sh_cmd, "python %s %s %d %d", cpPath, gSetting_system.com_ip, item_id, gSetting_system.Broadcast_volume);
		system(sh_cmd);
            
                syslog(LOG_DEBUG, "mag play time:%.3f < 0!\n", tt.elapsed());
                return 1;
	}
...
static void broadcast_msg_queue_init(void)
{
	...
	system("touch "BROADCAST_NEW_INFO_QUEUE_TMP_FILE);
	key = ftok(BROADCAST_NEW_INFO_QUEUE_TMP_FILE, 'a');
...
int broadcast_msg_send(u8 cmd, u32 param, char *led_msg)
{
	msg_event_t msg;
	int ret;
...
void *pthread_dev_handle2(void *arg)
{
	...
	//broadcast_msg_queue_init();
	signal(SIGPIPE,SIG_IGN);

	while (1)
	{
		ret = broadcast_trans_handle(gFd_dev);
                //syslog(LOG_DEBUG, "broadcast_trans_handle ret:%d\n", ret);
		//if (-99 == ret)  //time out
//                if (ret < 0)
//		{
//                    usleep(500 * 1000);
...
//
//		strcpy(g_ObjectStatus.szObjectCode,			"DEV_BRT_001");
...
//		{
                memcpy(led_msg, strMsg.c_str(), strMsg.size());
		return led_print_string(fd_dev, 0xF, led_msg);
//		}
//		else
//		{
