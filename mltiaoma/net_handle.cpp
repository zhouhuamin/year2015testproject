...
extern std::vector<structLcdMsg>            g_MsgVect;
...
int net_dev_register_handle(int server_fd)
{
    ...
    syslog(LOG_DEBUG, "result: %d, action: %d", p_reg_ack->reg_result, p_reg_ack->next_action);
    return 0;
}
...
int net_system_CTRL_handle(int server_fd, u8 *buf, int len)
{
    ...
    if (!strcmp(p_ctrl->msg_body, gSetting_system.SYS_CTRL_start_event_ID))
    {
        syslog(LOG_DEBUG, "ASK:\tStart gather!\n");
        g_StartEventSem.post();
    }
    else if (!strcmp(p_ctrl->msg_body, gSetting_system.SYS_CTRL_stop_event_ID))
    {
        syslog(LOG_DEBUG, "ASK:\tStop gather!\n");
