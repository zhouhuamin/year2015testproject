...
void ReadConfigInfo(struct structConfigInfo &configInfo)
{
	...
	TiXmlHandle SerialIPHandle 	= docHandle.FirstChildElement("ConfigInfo").ChildElement("SerialIP", 0).FirstChild();
	TiXmlHandle SerialPortHandle 	= docHandle.FirstChildElement("ConfigInfo").ChildElement("SerialPort", 0).FirstChild();
...
//=====================================
	...
	if (SerialIPHandle.Node() != NULL)
		configInfo.strKaKouIPHandle   = strKaKouIPHandle 			= SerialIPHandle.Text()->Value();

	if (SerialPortHandle.Node() != NULL)
		configInfo.strKaKouPortHandle = strKaKouPortHandle 		= SerialPortHandle.Text()->Value();
...
int main(int argc, char * argv[])
{
	signal(SIGALRM,SIG_IGN);
	signal(SIGPIPE,SIG_IGN);
	openlog("SimulatorSerialRecv", LOG_PID, LOG_LOCAL7);
	...
	if(!CheckSocketValid(nSocket))
	{
      ...
      if (ConnectSocket(nSocket, g_configInfo.strKaKouIPHandle.c_str(), nRemotePort) <= 0)
        {
		//	不能建立连接，直接返回
		// CloseSocket(nSocket);
		syslog(LOG_DEBUG, "Can't connect host\n");
