...
void GetContaOwnerData(std::vector<std::string> &contaOwnerVect)
{
	int i = 0;
	char szLine[256] = {0};
	FILE *pFile = fopen("/root/zhouhm/FutureVersion/SimulatorIO-Dalian/iomessage.txt", "r");
	if (pFile == NULL)
	{
		return;
	}
	else
	{
		do
		{
			...
			else
			{
				...
				std::string source			= szLine;
				std::string division		= " ";
				std::vector<std::string>	dest;
				SplitString(source, dest, division);
				copy(dest.begin(), dest.end(), back_inserter(contaOwnerVect));
			}
		} while (1);
	}
	fclose(pFile);
...
#define	nMaxLen 0x1000
	...
	std::vector<std::string> contaOwnerVect;
	...
	{
		char chRet  = StrToChar(contaOwnerVect[i].c_str());
		chVect.push_back(chRet);
	}
...
// 01 53 23 2F 0F 00 0C 00 36 43------------1
		for (int i = 0; i < chVect.size(); i += 10)
		{
			nSendLen = 10;
			nLen = SocketWrite(nSocket, &chVect[i], nSendLen, 30);	
			//	客户端数据
			if(nLen>0)
			{
				nLoopTotal=0;
