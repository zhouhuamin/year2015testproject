...
int  SocketRead_n(int nSocket,void * pBuffer,int nLen)
{
	int nRecvlen = 0;
	int nSum     = 0;
	do 
...
#define	nMaxLen 0x1000
	...
	while(!GlobalStopFlag&&!nSocketErrorFlag)
	{
		...
		// 0.
		nLen = SocketRead_n(nSocket, pBuffer, nRecvLen);	
		//	读取客户端数据
		...
		if (pBuffer[0] == 0x40 && pBuffer[1] == 0x00 && pBuffer[2] == 0x00 && pBuffer[3] == 0x00)
		{
			pBuffer[0] = 0x40;
			pBuffer[1] = 0x00;
			pBuffer[2] =  0x00;
		...
		else if (pBuffer[0] == 0x40 && pBuffer[1] == 0x04 && pBuffer[2] == 0x00 && pBuffer[3] == 0x01 && nFistFlag == 0)
		{
			pBuffer[0] = 0x40;
			pBuffer[1] = 0x04;
			pBuffer[2] =  0xFF;
		...
		else if (pBuffer[0] == 0x40 && pBuffer[1] == 0x04 && pBuffer[2] == 0x00 && pBuffer[3] == 0x01 && nFistFlag == 1)
		{
			pBuffer[0] = 0x40;
			pBuffer[1] = 0x04;
			pBuffer[2] =  0x00;
		...
		else if (pBuffer[0] == 0x40 && pBuffer[1] == 0x05 && pBuffer[2] == 0x00 && pBuffer[3] == 0x02)
		{
			pBuffer[0] = 0x40;
			pBuffer[1] = 0x05;
			pBuffer[2] =  0x00;
		...
		else if (pBuffer[0] == 0x40 && pBuffer[1] == 0x06 && pBuffer[2] == 0x00 && pBuffer[3] == 0x05)
		{
			pBuffer[0] = 0x40;
			pBuffer[1] = 0x06;
			pBuffer[2] =  0x00;
		...
		else if (pBuffer[0] == 0x40 && pBuffer[1] == 0x08 && pBuffer[2] == 0x00 && pBuffer[3] == 0x08)
		{
			pBuffer[0] = 0x40;
			pBuffer[1] = 0x08;
			pBuffer[2] =  0x00;
		...
		else if (pBuffer[0] == 0x40 && pBuffer[1] == 0x09 && pBuffer[2] == 0x00 && pBuffer[3] == 0x01)
		{
			pBuffer[0] =  0x40;
			pBuffer[1] =  0x09;
			pBuffer[2] =  0x00;
		...
		else if (pBuffer[0] == 0x40 && (BYTE)pBuffer[1] == 0xA6 && pBuffer[2] == 0x00 && pBuffer[3] == 0x01)
		{
			pBuffer[0] = 0x40;
			pBuffer[1] = 0xA6;
			pBuffer[2] =  0xDD;
...
int main(int argc, char * argv[])
{
	signal(SIGALRM,SIG_IGN);
	signal(SIGPIPE,SIG_IGN);
	openlog("SimulatorICCard", LOG_PID, LOG_LOCAL1);
