...
void *ArrivedEventThread(void *arg)
{
	...
	while (1)
	{
		...
		if (nFlag == 1)
		{
			...
			{
				syslog(LOG_DEBUG, "Arrived Read Count1=%d, Count2=%d, str1=%s, str2=%s\n", nCountValue1, nCountValue2, str1.c_str(), str2.c_str());
...
void *TimerEventThread(void *arg)
{
	...
	while (1)
	{
		...
		{
			if ((*it).second.nSendFlag == 1  &&  t - (*it).second.nTime > 3)
			{
				g_LabelMap.erase(it++);
			}
			else
