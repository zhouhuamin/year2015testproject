...
struct TestConfig
{
	...
	void save2(const std::string& filename, const std::string &AREA_ID, const std::string &CHNL_NO, const std::string &I_E_TYPE, const std::string &SEQ_NO);
	void save3(const std::string& filename, const std::string &AREA_ID, const std::string &CHNL_NO, const std::string &I_E_TYPE, const std::string &SEQ_NO);
};
...
void TestConfig::save2(const std::string& filename, const std::string &AREA_ID, const std::string &CHNL_NO, const std::string &I_E_TYPE, const std::string &SEQ_NO)
{
	...
	std::string EXCUTE_COMMAND	= "";
	if (I_E_TYPE == "I")
		EXCUTE_COMMAND	= "00000002000000000000";
	else
		EXCUTE_COMMAND	= "00000001000000000000";
//EXCUTE_COMMAND	= "00000002000000000000";
	...
	pt.put("COMMAND_INFO.SEAL.ESEAL_ID", "12900095");
	pt.put("COMMAND_INFO.SEAL.SEAL_KEY", "4159344504");


	pt.put("COMMAND_INFO.OP_TYPE", "2");
...
void TestConfig::save3(const std::string& filename, const std::string &AREA_ID, const std::string &CHNL_NO, const std::string &I_E_TYPE, const std::string &SEQ_NO)
{
	char szOP_REASON[1024] = {0};
	using boost::property_tree::ptree;
	ptree pt;
	...
	//std::string I_E_TYPE = "I";
	//std::string SEQ_NO	 = "20150801144651336141";
	std::string CHECK_RESULT	= "00000000000000000000";
	...
	pt.put("COMMAND_INFO.CHECK_RESULT", CHECK_RESULT);
	...
	pt.put("COMMAND_INFO.OP_HINT", title);
	pt.put("COMMAND_INFO.OP_ID", "");
...
*/
		...
		if(nLen>0 && pBuffer[8] == 0x32)
		{
...

			{
				...
				doc.Parse(pRecvXmlData);
				TiXmlHandle docHandle(&doc);
				TiXmlHandle GATHER_INFOHandle		= docHandle.FirstChildElement("COMMAND_INFO");
				if (GATHER_INFOHandle.Node() != NULL)
					AREA_ID 		= GATHER_INFOHandle.Element()->Attribute("AREA_ID");
				if (GATHER_INFOHandle.Node() != NULL)
					CHNL_NO 		= GATHER_INFOHandle.Element()->Attribute("CHNL_NO");
	...
	*/
				...
				strFileName += SEQ_NO;
				strFileName += ".xml";
				tc.save3(strFileName, AREA_ID, CHNL_NO, I_E_TYPE, SEQ_NO);
