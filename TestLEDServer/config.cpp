...
int set_parame_default(void)
{
	...
	strcpy(gSetting_system.SYS_CTRL_R_stop_event_ID, SYS_CTRL_R_STOP_EVENT_ID);
        ...
        strcpy(gSetting_system.szIC_WEL_EVENT, SYS_CTRL_IC_WELCOME_EVENT_ID);
...
int load_setting_from_xml(sSystem_setting *p_setting)
{
	...
	while ( NULL != cur_node)
	{
		...
		BAD_CAST "SYS_CTRL_WELCOME_EVENT_ID"))
		{
			str_tmp = xmlNodeGetContent(cur_node);

			sscanf((char *) str_tmp, "%s",
					gSetting_system.szIC_WEL_EVENT);
...
int save_setting_to_xml(sSystem_setting *p_setting)
{
	...
	BAD_CAST (str_tmp));
        ...
        sprintf(str_tmp, "%s", gSetting_system.szIC_WEL_EVENT);
	xmlNewChild(root_node, NULL, BAD_CAST "SYS_CTRL_WELCOME_EVENT_ID",
	BAD_CAST (str_tmp));
